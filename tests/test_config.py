...
""".strip(),
    ...
    assert cfg.p2pool.deactivating_escalate_signal is None
...
def test_deactivating_escalate_signal_opt_in(tmp_path: Path) -> None:
    path = tmp_path / "deckd.toml"
    path.write_text(
        """
[general]
...
def test_deactivating_escalate_signal_out_of_range(tmp_path: Path) -> None:
    path = tmp_path / "bad.toml"
    path.write_text(
        """
[general]
...
[p2pool]
unit = "p2pool.service"
deactivating_escalate_signal = 99
[onair]
server = "http://x"
register_interval = 0
[images]
...
""".strip(),
    ...
    with pytest.raises(ValueError, match="deactivating_escalate_signal"):
        load_config(path)
